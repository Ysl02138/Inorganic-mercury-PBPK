# ---
# title: "iHg - Markov chain Monte Carlo Calibration Diagnosis for human"
...
# This section will produce Supplemental Figure S4 and Figure S7
```{r, eval=F}
...
ggplot2::ggsave(filename = "plots/suppl/Figure_S4_traceplot_Human.eps", 
                plot = tp_Human, 
                device = cairo_ps, 
                dpi = 1200, 
                width = 15,
...
ggplot2::ggsave(filename = "plots/suppl/Figure_S7_density_Human.eps", 
                plot = dp_Human, 
                device = cairo_ps, 
                dpi = 1200, 
                width = 15,
...
# This section produces results for the posterior distributions shown in Supplemental Tables S7 and S10.  
#  
# From Supplemental Table S7, one can exponentiate each scaling coefficient parameter to obtain the posterior estimate as presented in Table 
# (e.g., exp(lnPLC) = exp(2.375) = 10.8, where PLC is the liver:blood partition coefficient for human).  
#  
# For Supplemental Table S10, the residual error geometric standard deviation was estimated as the exponential of 
...
```
# MPSRF
# This section will produce MPSRF values for Supplemental Figure S4 traceplot
```{r}
...
# This section will produce Supplemental Figure S10 plot
```{r}
...
ggsave("plots/suppl/Figure_S10_Correlation_matrix_Human.jpg", dpi = 600, height = 10, width = 15, units="in")
```
#
#
# ------------------------------------------------------------------------
...
#
#
# Correlation matrix of study-level posterior iHg-specific parameter ---- Hall 1995
# For Hall 1995, exposure scenario is a single IV dose of 0.025 ug Hg/kg/d for humans. 
# This study is labeled as Study 1.1, as it is listed as the first study in the human analysis. 
# See the MCMC input file in the "MCMC replication" folder for details  (e.g., iHgHuman_3365.in for details)
#
...
Human_corr[j,,M_pars] %>% matrix(nrow = sum_chains) %>%
  ...
  GGally::  ggcorr(nbreaks = 5, label = T, label_size = 4, label_round = 2, size = 4, label_alpha = TRUE) +
  theme_minimal() + labs(title = "Human") + theme(plot.title = element_text(size=24, hjust = 0.5)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), title = element_blank()) +   theme(plot.margin=grid::unit(c(0,0,0,0), "mm")) 
ggsave("plots/suppl/Human_Correlation_Rothstein_1960.jpg", dpi = 600, height = 10, width = 15, units="in")
```

# Correlation matrix of study-level posterior iHg-specific parameter ---- Rahola 1973
# For Rahola 1973, exposure scenario is a single oral of 0.09375 ug Hg/kg for humans.
# This study is labeled as Study 1.2, as it is listed as the second study in the human analysis. 
# See the MCMC input file in the "MCMC replication" folder for details  (e.g., iHgHuman_3365.in for details)
#
...
Human_corr[j,,M_pars] %>% matrix(nrow = sum_chains) %>%
  ...
  GGally::  ggcorr(nbreaks = 5, label = T, label_size = 4, label_round = 2, size = 4, label_alpha = TRUE) +
  theme_minimal() + labs(title = "Human") + theme(plot.title = element_text(size=24, hjust = 0.5)) + theme(axis.title.x = element_blank(), axis.title.y = element_blank(), title = element_blank()) +   theme(plot.margin=grid::unit(c(0,0,0,0), "mm")) 
ggsave("plots/suppl/Human_Correlation_Rahola_1973.jpg", dpi = 600, height = 10, width = 15, units="in")
```
